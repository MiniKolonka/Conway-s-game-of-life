...
class Field:
    ...
    def count_of_neighbors(self, diagonal_index: int, cell_index: int) -> int:
        ...
        upper_neighbors = [self.field_copy[upper_diagonal][upper_cell], self.field_copy[upper_diagonal][lower_cell], self.field_copy[upper_diagonal][cell_index]]
        lower_neighbors = [self.field_copy[lower_diahonal][upper_cell], self.field_copy[lower_diahonal][lower_cell], self.field_copy[lower_diahonal][cell_index]]
        middle_neighbors = [self.field_copy[diagonal_index][upper_cell], self.field_copy[diagonal_index][lower_cell]]

        return sum(map(int, upper_neighbors + lower_neighbors + middle_neighbors))

    def update_state(self) -> None:
        # Updates field
        self.field_copy = numpy.array(self.field)

        for index_of_diagonal, diagonal in enumerate(self.field_copy):
            for index_of_cell, cell in enumerate(diagonal):
                ...
                elif cell and neighbors not in (2, 3):
